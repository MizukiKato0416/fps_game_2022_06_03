...
//マクロ定義
//================================================
#define MODEL_COLLISIONE_SUBTRACTION	(1.0f)		//誤差
...
//デフォルトコンストラクタ
//================================================
CModelCollision::CModelCollision(CObject::PRIORITY Priority) :CObject(Priority)
{
...
//初期化処理
//================================================
HRESULT CModelCollision::Init(void)
{
	//親の設定
	...
	//オブジェクトの種類の設定
	SetObjType(CObject::OBJTYPE::MODEL_COLLISION);
...
//終了処理
//================================================
void CModelCollision::Uninit(void)
{
	Release();
}
...
//更新処理
//================================================
void CModelCollision::Update(void)
{
...
//描画処理
//================================================
void CModelCollision::Draw(void)
{
	//デバイスのポインタ
...
//生成処理
//================================================
CModelCollision *CModelCollision::Create(const D3DXVECTOR3 &pos, const D3DXVECTOR3 &rot, const string &sPas, CModel *pParent, const bool &Collision)
{
	//インスタンスの生成
	CModelCollision *pModelCollision = nullptr;
	if (pModelCollision == nullptr)
	{
		pModelCollision = new CModelCollision;
		if (pModelCollision != nullptr)
		{
			pModelCollision->m_bCollision = Collision;
			pModelCollision->m_rot = rot;
			pModelCollision->m_pParent = pParent;
			pModelCollision->m_pos = pos;
			//モデルの生成
			pModelCollision->m_pModel = CModel::Create(D3DXVECTOR3(0.0f, 0.0f, 0.0f), D3DXVECTOR3(0.0f, 0.0f, 0.0f), sPas);
			pModelCollision->Init();
		}
	}
	return pModelCollision;
}

//================================================
//当たり判定処理
//================================================
bool CModelCollision::SimpleCollision(CObject *pObject)
{
	...
	for (int count_object = 0; count_object < object_size; count_object++)
	{
		if (object[count_object]->GetObjType() == CObject::OBJTYPE::MODEL)
		{
			//モデルシングルにキャスト
			CModelCollision *pModelCollision = nullptr;
			pModelCollision = (CModelCollision*)object[count_object];
			...
			//モデルの位置・サイズ取得
			posModel = pModelCollision->GetPos();
			sizeModel = pModelCollision->GetSize();
...
//回転させていい当たり判定処理
//================================================
int CModelCollision::Collision(CObject *pObject)
{
	//着地しているかどうか
	int nHit = 0;
	...
	for (int count_object = 0; count_object < object_size; count_object++)
	{
		if (object[count_object]->GetObjType() == CObject::OBJTYPE::MODEL)
		{
			//pObjectをCModelCollisionにキャスト
			CModelCollision *pModelCollision = (CModelCollision*)object[count_object];

			//当たり判定をする設定なら
			if (pModelCollision->m_bCollision == true)
			{
				//8頂点の取得
				D3DXVECTOR3 vtxPos[MODEL_VTX];
				for (int nCntVtx = 0; nCntVtx < MODEL_VTX; nCntVtx++)
				{
					vtxPos[nCntVtx] = pModelCollision->m_pModel->GetVtxPos(nCntVtx);
				}
				...
				//8頂点のワールドマトリックスを取得
				D3DXMATRIX *pVtxMtxWorld = pModelCollision->m_pModel->GetVtxMtxWorld();
				//8頂点の設定
				for (int nCntVtx = 0; nCntVtx < MODEL_VTX; nCntVtx++, pVtxMtxWorld++)
				{
					pModelCollision->m_pModel->SetVtxMtxWorld(vtxPos[nCntVtx], nCntVtx);
				...
				if (fVecDot[0] <= 0.0f && fVecDot[1] <= 0.0f && fVecDot[3] <= 0.0f && fVecDot[4] <= 0.0f && fVecDot[5] <= 0.0f &&
					fVecDotOld[0] > -MODEL_COLLISIONE_SUBTRACTION)
				{//奥の面
					//ムーブベクトルを求める
					...
					pObject->SetPos(objectPos);
						 ...
						 fVecDotOld[2] > -MODEL_COLLISIONE_SUBTRACTION)
				{//前の面
					//ムーブベクトルを求める
					D3DXVECTOR3 moveVec = pos - posOld;
				...
				if (fVecDot[0] <= 0.0f && fVecDot[1] <= 0.0f && fVecDot[2] <= 0.0f && fVecDot[4] <= 0.0f && fVecDot[5] <= 0.0f &&
					fVecDotOld[1] > -MODEL_COLLISIONE_SUBTRACTION)
				{//右の面
					//ムーブベクトルを求める
					D3DXVECTOR3 moveVec = pos - posOld;
					...
					pObject->SetPos(objectPos);
						 ...
						 fVecDotOld[3] > -MODEL_COLLISIONE_SUBTRACTION)
				{//左の面
					//ムーブベクトルを求める
					D3DXVECTOR3 moveVec = pos - posOld;
...
//回転させていい当たり判定処理
//================================================
int CModelCollision::Collision(CObject * pObject, const float &fRadius, const float &fHeight)
{
	//どこからあたったか
	int nHit = 0;
...
//ただの衝突判定
//================================================
int CModelCollision::CollisionAny(CObject *pObject)
{
	//オブジェクト情報を入れるポインタ
	vector<CObject*> object;
...
//ワールドマトリックス設定処理
//================================================
void CModelCollision::SetMtxWorld(void)
{
	//デバイスのポインタ
	LPDIRECT3DDEVICE9 pDevice;
	//デバイスの取得
